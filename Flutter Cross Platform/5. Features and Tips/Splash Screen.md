> Concept of Delaying execution of Navigation to the home screen

- Depends on : `Future.delay`( Duration()  ,  ( ){ order of Navigation })
- should be executed in the `initState`

## Types of Splash:
- Basic (context)
- Animation (Use animated `Json` + content + not Native style)=> requires [`lottie`](https://pub.dev/packages/lottie/example)package
- Native (image +native style+ no animation + no content) => requires [`flutter_native_splash`](https://pub.dev/packages/flutter_native_splash)
- Advanced(animation gif + native style)
## Navigate and Delay: 
```dart
void initState() {
...
```
## Basic Way:
> any simple screen you want to add

## Animation:
> by adding the `Lottie` package

>[!Note] the animation should be in the form of `Json`

```dart
Center(child: Lottie.asset(Assets.imagesSplashAnimation))
```
## Native:
> by adding the flutter native splash package
#### Requires:
- create a file called `flutter_native_splash.yaml`
- take the code copy in the package and put it in this file
- change background Color in file
- change used image
- check that android , IOs are true in platform section  
...
```
- make sure to edit the main function:
```dart
...
### Steps:
- add the packages: 
	- [`animated_splash`](https://pub.dev/packages/animated_splash_screen/install)
	- [`flutter_native_splash`](https://pub.dev/packages/flutter_native_splash)
- add the widget in the home field of `MaterialApp`:
```dart
AnimatedSplashScreen(
	splash: 'assets/test/Netflix Gifs.gif',  //the annimation you downloaded
...
```
- follow the same steps from the Native method
- run your project